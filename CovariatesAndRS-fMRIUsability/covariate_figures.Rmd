...
**Figure 1b. Motion quality control leads to dramatic reductions in sample size.** The proportion of children in each diagnosis group whose scans were included (yellow) and excluded (lavender) using the strict (left) and lenient (right panel) gross motion QC. A larger proportion of children in the autism spectrum disorder (ASD) group were excluded compared to typically developing (TD) children using lenient motion QC ($\chi^2$=10.3, df = 1, p=0.001) and strict ($\chi^2$=19.9, df = 1, p<0.00001).
...
#### Checking positivity assumption for DRTMLE
```{r}
ados %>% filter(Motion.Exclusion.Level=="Lenient" & Included=="Excluded") %>% select(value) %>% max()
ados %>% filter(Motion.Exclusion.Level=="Lenient" & Included=="Included") %>% select(value) %>% max()
```

The highest ADOS score among included children using leninet motion QC is 23; among all children, 26.
