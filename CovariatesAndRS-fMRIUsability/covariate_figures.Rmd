...
#### Create Diagnosis variable with 3 levels: Autism-only, Autism+ADHD, and TD
```{r}
dat3$Diagnosis3Levels=rep(NA,length = nrow(dat3))

dat3$Diagnosis3Levels[dat3$ADHD_Secondary=="1"] = "ASD+ADHD"
dat3$Diagnosis3Levels[dat3$ADHD_Secondary==0 & dat3$PrimaryDiagnosis=="ASD"] = "ASD-only"
dat3$Diagnosis3Levels[dat3$PrimaryDiagnosis=="TD"] = "TD"

dat3$Diagnosis3Levels <- factor(dat3$Diagnosis3Levels, levels = c("TD", "ASD-only", "ASD+ADHD"))
...
allVariables = c("ID", "PrimaryDiagnosis", "AgeAtScan", "Ciric_length", "KKI_criteria", "noExclusion", 
                 ...
                 "ADHD_Secondary", "Diagnosis3Levels", "SES.Family", 
...
#### Limit initial data set to complete cases and check consistency of diagnosis variables
```{r}

dat3 <- filter(dat3, CompletePredictorCases==1)

table(dat3$ADHD_Secondary)
table(dat3$Diagnosis3Levels)
```
49 ASD-only + 353 TD = 402 ADHD_Secondary = 0
...
tabASDonly<- tableby(Motion.Exclusion.Level~Included, data=filter(completeCases,
                                                                    Diagnosis3Levels=="ASD-only"))

summary(tabASDonly,  
        title = "Proportion of ASD-only complete cases included/excluded",
        digits=0, digits.p=4,digits.pct=1, numeric.simplify=TRUE, total=FALSE, test=FALSE)

tabASDco<- tableby(Motion.Exclusion.Level~Included, data=filter(completeCases,
                                                                    Diagnosis3Levels=="ASD+ADHD"))

summary(tabASDco,  
        title = "Proportion of ASD+ADHD complete cases included/excluded",
        digits=0, digits.p=4,digits.pct=1, numeric.simplify=TRUE, total=FALSE, test=FALSE)
...
graph <- grViz("digraph flowchart {
      ...
      node [fontname = Helvetica, shape = rectangle, style = rounded, penwidth = 3, fontsize = 18, fixedsize = TRUE, width = 3, height = 1]    
#attempted
      tab1 [label = '@@1']
...
```{r, tidy=TRUE, fig.width=8, fig.height=2}
...
motion$Diagnosis3Levels <- relevel(motion$Diagnosis3Levels, "TD")

motion <- group_by(motion, Diagnosis3Levels, Motion.Exclusion.Level, Included)

dx_proportions <- ggplot(motion, aes(x = Diagnosis3Levels, fill = Included))+
...
#nested models
ex_chisq <- exNest %>% 
  mutate(stats = map(data, ~broom::tidy(chisq.test(.x$Diagnosis3Levels, .x$Included)))) %>% 
  unnest(stats) 
...
```{r}
comorbid <- filter(motion, Diagnosis3Levels!="TD")
comorbid$Diagnosis3Levels <- droplevels(comorbid$Diagnosis3Levels)
levels(comorbid$Diagnosis3Levels)

#comorbid <- filter(comorbid, !is.na(Diagnosis3Levels))

coNest <- tibble(comorbid) %>% 
  select(c("Diagnosis3Levels", "CurrentlyOnStimulants",
           "Motion.Exclusion.Level", "Included")) %>% 
  reshape2::melt(id.vars = c("Motion.Exclusion.Level", "Included"),
       measure.vars = c("Diagnosis3Levels", "CurrentlyOnStimulants")) %>% 
...
#### 1d.3 TD vs ASD only 
```{r}
asdTD <- filter(motion, Diagnosis3Levels!="ASD+ADHD")
asdTD$Diagnosis3Levels <- droplevels(asdTD$Diagnosis3Levels)
levels(asdTD$Diagnosis3Levels)

asdNest <- tibble(asdTD) %>% 
  select(c("Diagnosis3Levels",
           "Motion.Exclusion.Level", "Included")) %>% 
  reshape2::melt(id.vars = c("Motion.Exclusion.Level", "Included"),
       measure.vars = c("Diagnosis3Levels")) %>% 
  group_by(Motion.Exclusion.Level) %>% 
  tidyr::nest()
...
#### 1d.4 TD vs ASD+ADHD
```{r}
coTD <- filter(motion, Diagnosis3Levels!="ASD")
coTD$Diagnosis3Levels <- droplevels(coTD$Diagnosis3Levels)
levels(coTD$Diagnosis3Levels)
...
props <- cowplot::plot_grid(dx_proportions, stim_proportions, ncol=2, 
                   rel_widths=c(3/5, 2/5), labels = c("","", ""))
