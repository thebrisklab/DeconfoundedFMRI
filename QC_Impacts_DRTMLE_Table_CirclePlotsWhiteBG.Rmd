...
myChordDiagram = function(data,varname,alpha=0.20,alphaname,title) {
  ...
  oDATA$NewOrder[oDATA$Group=="SC"]=4
...
results.all=NULL
for (i in 1:200) {
  load(paste0('../Results/Results_19November2021/ic30_pc85_glm_gam_drtmle_seed',i,'.RData'))
...
results.ave$td.p.naive = 2*(1-pnorm(abs(results.ave$z.stat.TD.naive)))
results.ave$td.p.naive.fdr = round(p.adjust(results.ave$td.p.naive,method='BH'), digits = 3)
results.ave$td.p.SL = 2*(1-pnorm(abs(results.ave$z.stat.TD.SL)))
results.ave$td.p.SL.fdr = round(p.adjust(results.ave$td.p.SL,method='BH'), digits = 3)

```

#### Determine edges showing a significant group difference FDR=.05
```{r}
...
results.sigEdges = results.ave[fdr20.edges.SL,c(1,2:3, 7:9,13,15, 17)]

results.sigEdges <-   results.sigEdges[order(results.sigEdges$p.SL.fdr), ]

names(results.sigEdges)[1] = "Edge"
names(results.sigEdges)[2] = "naive ASD mean"
names(results.sigEdges)[3] = "naive TD mean"
names(results.sigEdges)[4] = "naive ASD-TD Z"
names(results.sigEdges)[5] = "DRTMLE ASD mean"
names(results.sigEdges)[6] = "DRTMLE TD mean"
names(results.sigEdges)[7] = "DRTMLE ASD-TD Z"
names(results.sigEdges)[8] = "naive FDR-adjusted p"
names(results.sigEdges)[9] = "DRTMLE FDR-adjusted p"
